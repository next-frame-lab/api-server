...
import wisoft.nextframe.payment.application.payment.outbox.cancel.ReservationCancelOutboxRepository;
import wisoft.nextframe.payment.application.payment.outbox.cancel.ReservationCancelOutboxTarget;

@Slf4j
@Repository
@RequiredArgsConstructor
public class ReservationCancelOutboxRepositoryImpl implements ReservationCancelOutboxRepository {

	private final JpaReservationCancelOutboxRepository jpa;
	...
	public void upsertPending(UUID paymentId, UUID reservationId, String lastError, LocalDateTime now) {
		...
		var entity = jpa.findByReservationId(reservationId).orElseGet(() -> {
			var e = new ReservationCancelOutboxEntity();
	...
	@Override
	@Transactional
	public void markSuccess(UUID reservationId, LocalDateTime now) {
	...
	@Override
	public List<ReservationCancelOutboxTarget> findTargets(LocalDateTime now, int limit) {
		return jpa.findReadyToRetry(now, PageRequest.of(0, limit))
			.stream()
			.map(e -> new ReservationCancelOutboxTarget(
				e.getReservationId(),
				e.getPaymentId()
			))
			.toList();
